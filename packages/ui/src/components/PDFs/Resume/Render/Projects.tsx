...
export const Projects = ({ data, setMax, keep, showPadding, noOverflow, scale }: IProp) => {
  ...
  useEffect(() => {
    if (!!pdf?.current && !!!noOverflow) {
      ...
      if (keep && setMax) {
        ...
        setMax((prev) => ({
          ...prev,
          projects: check === 0 ? prev.projects : check < 0 ? prev.projects + check : prev.projects + over.length
        }));
