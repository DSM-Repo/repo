      ...
      {open && (
        ...
        >
          <div className="flex gap-2 items-center">
            <Icon name="Arrow" className="cursor-pointer" rotate="left" id="left" onClick={handleClick} />
            <span className="text-body3">
              {sel.year}. {sel.month}
            </span>
            <Icon name="Arrow" className="cursor-pointer" rotate="right" id="right" onClick={handleClick} />
          </div>
          <div className="flex flex-row flex-wrap gap-[5px] w-full">
            {dateTypes.map((i) => (
              <span key={i} className="w-[30px] h-[30px] text-center content-center text-body5 text-gray-300 inline cursor-pointer">
                {i}
              </span>
            ))}
            {startDate.map((_, j) => (
              <span key={j} className="w-[30px] h-[30px] inline" />
            ))}
