...
const Layout = ({ children }: ILProp) => {
  return (
    <div className="w-[842px] h-[1191px] bg-white relative col-flex items-center p-6 overflow-hidden">
      <div className="w-full bg-gray-100 h-[2px]" />
      {children}
    </div>
  );
};
export const Cover = ({ year, grade }: IProp) => {
  const { data: history } = histories();

  return (
    <>
      <Layout>
        <div className="px-16 col-flex absolute top-40 z-30 gap-4 left-5">
      ...
      <Layout>
        ...
        <div className="col-flex absolute left-0 px-20 top-[720px] gap-14">
          <span className="text-black font-bold text-[32px]">
            Repo 뷰어 사용 방법
          </span>
          <img
            src="/Usage.png"
            className="border-gray-100 border-[15px] rounded-[25px]"
          />
