...
export const Item = ({ fieldMethod: { remove, swap }, index }: IProp) => {
  ...
  return (
    <Box>
      ...
      <File size="large" label="로고" placeholder="로고" accept="image/*" value={watch(`${name}.logo`)} onChange={handleLogo} onDelete={() => fileReomoveMutate(`?url=${getValues(`${name}.logo`)?.image_path}`, { onSuccess: () => setValue(`${name}.logo`, undefined) })} />
